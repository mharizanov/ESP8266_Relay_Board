...
static void report(config *cfg) {
  ...
  fprintf(cfg->out->fd == STDOUT_FILENO ? stderr : stdout, "%s %0.2f %%\t %zd -> %zd (-w %u -l %u)\n", cfg->in_fname,
          100.0 - (100.0 * outb) / inb, inb, outb, cfg->window_sz2, cfg->lookahead_sz2);
}
...
int main(int argc, char **argv) {
  ...
  if (0 == strcmp(cfg.in_fname, cfg.out_fname) && (0 != strcmp("-", cfg.in_fname))) {
