...
// The static files with the following extensions from the HTML folder will be compressed and served with GZIP
// compression Add any other file types you want compressed here (and don't forget to modify the Makefile too)
#ifdef GZIP_COMPRESSION
static const char *gzippedFileTypes[] = {"js", "html", "css", NULL};

static const char gzipNonSupportedMessage[] =
    "<html><head></head><body>Your browser does not accept gzip-compressed data.</body></html>";
#endif
  ...
  while (1) {
    ...
    while (builtInUrls[i].url != NULL) {
      ...
      if (builtInUrls[i].url[os_strlen(builtInUrls[i].url) - 1] == '*' &&
          os_strncmp(builtInUrls[i].url, conn->url, os_strlen(builtInUrls[i].url) - 1) == 0)
        match = 1;
      if (match) {
...
static void ICACHE_FLASH_ATTR httpdRecvCb(void *arg, char *data, unsigned short len) {
  ...
  // This is slightly evil/dirty: we abuse conn->post->len as a state variable for where in the http communications we
  // are: <0 (-1): Post len unknown because we're still receiving headers
  //==0: No post data
