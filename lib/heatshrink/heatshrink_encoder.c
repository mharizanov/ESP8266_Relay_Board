...
static const char *state_names[] = {
    "not_full",       "filled",          "search",       "yield_tag_bit", "yield_literal",
    "yield_br_index", "yield_br_length", "save_backlog", "flush_bits",    "done",
};
#else
#define LOG(...)  /* no-op */
...
heatshrink_encoder *heatshrink_encoder_alloc(uint8_t window_sz2, uint8_t lookahead_sz2) {
  if ((window_sz2 < HEATSHRINK_MIN_WINDOW_BITS) || (window_sz2 > HEATSHRINK_MAX_WINDOW_BITS) ||
      (lookahead_sz2 < HEATSHRINK_MIN_LOOKAHEAD_BITS) || (lookahead_sz2 > window_sz2)) {
    return NULL;
  }
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start, uint16_t end, const uint16_t maxlen,
...
HSE_poll_res heatshrink_encoder_poll(heatshrink_encoder *hse, uint8_t *out_buf, size_t out_buf_size,
                                     size_t *output_size) {
...
static HSE_state st_step_search(heatshrink_encoder *hse) {
  ...
  uint16_t match_pos = find_longest_match(hse, start, end, max_possible, &match_length);
...
static uint16_t get_input_offset(heatshrink_encoder *hse) { return get_input_buffer_size(hse); }
...
static int is_finishing(heatshrink_encoder *hse) { return hse->flags & FLAG_IS_FINISHING; }

static int backlog_is_partial(heatshrink_encoder *hse) { return hse->flags & FLAG_BACKLOG_IS_PARTIAL; }

static int backlog_is_filled(heatshrink_encoder *hse) { return hse->flags & FLAG_BACKLOG_IS_FILLED; }

static int on_final_literal(heatshrink_encoder *hse) { return hse->flags & FLAG_ON_FINAL_LITERAL; }

static int has_literal(heatshrink_encoder *hse) { return (hse->flags & FLAG_HAS_LITERAL); }

static int can_take_byte(output_info *oi) { return *oi->output_size < oi->buf_size; }
