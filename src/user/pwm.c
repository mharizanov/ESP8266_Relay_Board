...
#define US_TO_RTC_TIMER_TICKS(t)                                                                                       \
  ((t) ? (((t) > 0x35A) ? (((t) >> 2) * ((APB_CLK_FREQ >> 4) / 250000) + ((t)&0x3) * ((APB_CLK_FREQ >> 4) / 1000000))  \
 ...
 * Returns      : NONE
 *******************************************************************************/
uint8 ICACHE_FLASH_ATTR pwm_get_duty(uint8 channel) { return pwm.duty[channel]; }
 ...
 *******************************************************************************/
uint16 ICACHE_FLASH_ATTR pwm_get_freq(void) { return pwm.freq; }
...
LOCAL void pwm_tim1_intr_handler(void) {
  ...
  if (pwm_current_channel == (*pwm_channel - 1)) {
    ...
    gpio_output_set(pwm_single[*pwm_channel - 1].gpio_set, pwm_single[*pwm_channel - 1].gpio_clear, pwm_gpio, 0);
    ...
    gpio_output_set(pwm_single[pwm_current_channel].gpio_set, pwm_single[pwm_current_channel].gpio_clear, pwm_gpio, 0);
