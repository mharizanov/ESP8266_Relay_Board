...
function update() {
  ...
  if (Number(thermostat.temperature) == -999.9 && thermostat.thermostat1_input == 3) {
    $(".zone-temperature")
        .html("--.-" +
              "&deg;C" +
              "<BR><B>No temperature reading received via MQTT.</B>" +
              "<BR>Check remote temperature sender is working and MQTT broker running on " + thermostat.mqtthost + " ");
  } else if (Number(thermostat.temperature) == -999.9) {
    $(".zone-temperature")
        .html("--.-" +
              "&deg;C");
  } else {
